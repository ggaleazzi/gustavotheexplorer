...
let score, currentScore, activePlayer, playing, dicestotal, globalWinPoints;
...
const init = function () {
  ...
  btnPoints.classList.remove('hidden');
...
btnWin.addEventListener('click', function () {
  btnPoints.classList.add('hidden');
...
function funcwinpoints() {
  ...
  globalWinPoints = quantity;
  console.log(globalWinPoints);
}
...
btnHold.addEventListener('click', function () {
  if (playing) {
    ...
    if (score[activePlayer] >= globalWinPoints) {
      // finish the game
      playing = false;
      diceEl.classList.add('hidden');
      dice2El.classList.add('hidden');
